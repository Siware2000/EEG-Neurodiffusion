...
Explicit topology preservation metrics:
...
# **Quick Start**
## **Installation**
git clone https://github.com/Siware2000/EEG-Neurodiffusion.git
cd EEG-Neurodiffusion
pip install -r requirements.txt
# **Reproducing Paper Results (No Retraining)** 
## All reported results can be reproduced using frozen checkpoints.
# Generate ablation & statistics tables
python src/scripts/journal_stats_from_classification_report.py

# Degradation & effect size analysis
python src/scripts/journal_degradation_analysis.py
python src/scripts/journal_effect_size_tables.py

# Confusion matrices and ROC curves
python src/scripts/eval_confusion_matrices.py
python src/scripts/eval_roc_curves.py

# ViT attention rollout visualizations
python src/scripts/eval_attention_rollout.py

# Topology preservation analysis
python src/scripts/run_topology_eval.py
python src/scripts/summarize_topology.py
...
## **Current Limitations**
Limited to static topomaps (no temporal dynamics)

Focused on 64-channel EEG configurations

Computational cost of diffusion sampling

Subject-independent but task-specific evaluation
## **Future Directions** 
Topology-aware diffusion losses

Electrode-graph or source-space diffusion

Temporal diffusion of topomap sequences
